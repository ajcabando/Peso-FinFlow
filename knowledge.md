...
- **GitHub**: repo `ajcabando/FinFlow` is **PRIVATE** → GitHub Pages unavailable (needs public or paid plan). `gh-pages` branch with `privacy-policy.html` is pushed and ready — flipping the repo public activates Pages at `https://ajcabando.github.io/FinFlow/privacy-policy.html`. gh CLI is authenticated as ajcabando (release/Pages/API all work).
